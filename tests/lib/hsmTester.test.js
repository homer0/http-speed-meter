...
describe('HsmTester', () => {
    ...
    it('should be able to print out the resoluts on the console', () => {
        ...
        .then(() => {
            ...
            expect(mock.calls[5][0]).toMatch(RegExp(
                `${axiosPackage.name}@${axiosPackage.version}\\W+█+ \\d(?:(\\.\\d)?)s`
            ));
            expect(mock.calls[6][0]).toMatch(RegExp(
                `${gotPackage.name}@${gotPackage.version}\\W+█+ \\d(?:(\\.\\d)?)s`
            ));
            expect(mock.calls[7][0].trim()).toBe('');
            expect(mock.calls[8][0]).toMatch(/test: text/i);
            expect(mock.calls[9][0]).toMatch(RegExp(
                `${axiosPackage.name}@${axiosPackage.version}\\W+█+ \\d(?:(\\.\\d)?)s`
            ));
            expect(mock.calls[10][0]).toMatch(RegExp(
                `${gotPackage.name}@${gotPackage.version}\\W+█+ \\d(?:(\\.\\d)?)s`
            ));
