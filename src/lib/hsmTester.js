...
class HsmTester {
   ...
   * @param {string} testsPath     The path, relative to where you are running the script,
   *                               in which all the test files are stored.
   * @param {Object} [options={}]  Optional. Custom options that overwrite the class
   *                               defaults.
   * @property {number} iterations       Optional. The number of iterations the tests must
   *                                     run.
   *                                     `1` by default.
   * @property {string} mock             Optional. The path, relative to where you are
   *                                     running the script, of a results mock file.
   *                                     This is usefull for coding the output.
   * @property {number} maxColumns       Optional. The maxium number of columns a chart
   *                                     bar can take on the results output. `100` by
   *                                     default.
   * @property {Object} normalizedNames  Optional. The titles you want to see for each
   *                                     test type;
   *                                     for example: `{ json: 'JSON', raw: 'Text'}`.
   * @property {Array}  colors           Optional. A list of the different colors the
   *                                     output can use. They need to be supported by
   *                                     `chalk@^2.0.0`.
   * @returns {HsmTester}
   */
  constructor(testsURL, testsPath, options = {}) {
    ...
    /**
     * A dictionary where the keys are the test names and the values are the absolute path
     * to their files.
     *
     * @type {Object}
     */
    ...
    /**
     * Once the tests run, this dictionary will have all the information of the tests.
     * The key will be the test name.
     *
     * @type {Object}
     */
    this.results = {};
     ...
     * @type {number}
     */
    this.maxColumns = options.maxColumns || DEFAULT_MAX_COLUMN;
    /**
    ...
    /**
     * When you call `getColor`, this counter will be incremented and the value of
     * `this.colors`
     * at that index will be returned; if there's no color available at that index,
     * the counter will be reseted and the first color will be returned instead.
     *
     * @type {number}
     * @ignore
     */
   ...
   * Run the tests.
   *
   * @param {string | RegExp} [name='']  Optional. By default, it runs all the tests it
   *                                     found at the moment it was initialized, but if
   *                                     this parameter is a `string`, it will only run
   *                                     the test which name is equal to the string; but
   *                                     if the parameter is a regular expression,
   *                                     it will only run tests that match that
   *                                     expression.
   * @returns {Promise<HsmTester, Error>} If all the tests successfully ran, then the
   *                                      promise will be resolved with a reference to
   *                                      this instance (for chaining purposes); but if
   *                                      something goes wrong with any of the tests,
   ...
   *
   * @returns {Object} A dictionary with a extension-less filenames as keys and the
   *                   absolute paths as values.
   */
  _findAllTheTests() {
    const tests = {};
   ...
   * @returns {Object} The request type infromation.
   * @ignore
   */
  ...
  /**
   * Get a `chalk` color to use on the output. This method uses an internal counter in
   * order to give you a new one from the `this.colors` list each time it's called. If it
   * runs out of colors, it will start from the first one again.
   *
   ...
   * @property {string} name        The name of the package's according to its
   *                                `package.json`.
   * @property {string} version     The package version.
   * @property {string} repository  The package repository URL.
   * @returns {Object} A dictionary with the basic information.
  ...
  _getInfo(depName) {
    ...
    const { name, version, repository } = JSON.parse(
      fs.readFileSync(path.join(process.cwd(), `node_modules/${depName}/package.json`)),
    );
  ...
  /**
   * The _"real show method"_. It takes all the processed information from `showResults`
   * and print the report on the console.
   *
  ...
  /**
   * The _"real"_ `run` method. The previous one checks if there the results are mocked
   * and if they are, it resolves the Promise without doing anything, but if it needs to
   * call the real tests, it returns this method.
   * This method first filters the list of tests it will run, then turns the spinner on
   * and triggers all the iterations for each of the tests it needs to run. Once they are
   * all resolved (by the magic of `Promise.all` inside `Promise.all`) it, once again,
   * resolves the main promise with the instance reference (for chaining purposes).
   *
   * @param {string | RegExp} name  Optional. By default, it runs all the tests it found
   *                                at the moment it was initialized, but if this
   *                                parameter is a `string`, it will only run the test
   *                                which name is equal to the string; but if the
   ...
   * output it will either add it to the results or reject the returned Promise.
   *
  ...
  /**
   * Given a path for a mock file, this method will try to read it, parse it and overwrite
   * the main `this.results` dictionary with its values.
   *
   * @param {string} filepath  The path to the mock file. Relative to where you are
   *                           running the script.
   * @returns {boolean} Whether or not the results were mocked.
